...
echo ""
echo "=== 项目文件检查 ==="
for file in "manage.sh" "README.md" ".gitignore" "docker-compose.yml" "install.sh"; do
    [ -f "$file" ]
    test_case "$file 存在" "$?"
done

for dir in "scripts" "api" "examples" "templates" "nginx"; do
    [ -d "$dir" ]
    test_case "$dir/ 目录存在" "$?"
done
...
echo "=== 脚本权限检查 ==="
[ -x "manage.sh" ]
test_case "manage.sh 可执行" "$?"

[ -x "scripts/status.sh" ]
test_case "scripts/status.sh 可执行" "$?"

[ -x "api/searxng_api.py" ]
test_case "api/searxng_api.py 可执行" "$?"
